...
module.exports = async (client, message, now, config) => {
    if (message.body.startsWith(`${config.prefix}pesquisar`)) {
        imageSearch.search(message.body.slice(11))
            .then(async image => {
                try {
                    ...
                    console.log("\x1b[32m", `[LOG] Imagem enviada com descrição em: ${Date.now() - now}ms`);
                } catch {
                    if (thumbnailNotUndefined(image)) {
                        ...
                        console.log("\x1b[33m", `[LOG] thumbnail enviada em: ${Date.now() - now}ms`);
                    } else {
                        await client.reply(message.from, `Nenhum resultado encontrado para: ${message.body.slice(11)}`, message.id);
                        console.log("\x1b[31m", "[LOG] Imagem não econtrada");
                    }
                }
            })
            .catch(async error => {
                await client.reply(message.from, `Nenhum resultado encontrado para: ${message.body.slice(11)}`, message.id);
                console.error(error);
                console.log("\x1b[31m", "[LOG] Imagem não encontrada");
            });
    }
};
